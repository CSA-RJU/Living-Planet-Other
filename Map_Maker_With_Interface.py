...
def Menu():
    global game_map, map_size_x, map_size_y, player_x, player_y
    print('''| 1: Create Map
| 2: Draw Map
| 3: Load Map
| 4: Save Map''')
...
def Man_Map_Maker():
    ...
    player_x = input("Where will the player be? (x position) |")
    player_y = input("Where will the player be? (y position) |")
    try:
        game_map[int(player_position_y) - 1][int(player_position_x) - 1] = "[p]"
    except(IndexError):
...
def Place_Obstacle(): #Places an obstacle at the given coordinates.
    ...
    print("")
    os.system('cls')
    item = input("What do you want toplace? |") #What ever is typed here will be the obstacle as a string.
...
def Load_Map():
    ...
    player_x = int(linesplit[2])
    player_y = int(linesplit[3])
...
def Save_Map():
    ...
    save_name = input("What will you call the level? |")
    map_save = open((f'{save_name}.LPM'), "w")
    map_save.write(f'{map_size_x},{map_size_y}, {player_x},{player_y} \n')
